<template>
  <div class="sign-up">
      ...
      <div class="input-group">
        <span class="iconfont icon-yonghu left-icon"/><input type="text" placeholder="请输入用户名" maxlength="20" v-model="name" v-focus-parent>
    ...
    <div class="button-group">
      <button :disabled="buttonDisabled" :class="{active: !buttonDisabled}" @click="signUp"><span v-show="!submitLoading">注册</span><f-fade-spinner v-show="submitLoading"/></button>
    </div>
    <div class="button-group">
      <button @click="toLogin" class="button-white">去登录</button>
    </div>
  </div>
</template>
<script>
import FadeSpinner from 'components/FadeSpinner';
import { signUp } from 'src/api/index';
export default {
  data() {
    return {
    	name: '',
